...
import { overlayForShelf } from "@/lib/thread-overlay";
...
export function ThreadInbox({
  ...
  const { pinned, await: awaitRows, done: doneRows, nextAction, waiting } = shelves;
  ...
  const cardFor = (row: VisibleInboxRow, shelf: ActiveThreadShelf) => {
    ...
    return (
      <ThreadCard
        ...
        statusOverlay={overlayForShelf(row.node.shelf)}
        command={command}
        now={now}
      />
    );
          ...
          <div className="gtd-sticky-band">
            {awaitRows.length > 0 ? (
              <Shelf
                ...
                count={awaitRows.length}
                isCompactViewport={isCompactViewport}
              >
                {awaitRows.map((row) => cardFor(row, "await"))}
              </Shelf>
            ) : null}
            {doneRows.length > 0 ? (
              <Shelf
            ...
            shelfThreads.length > 0 ? (
              <Shelf
                ...
                {...(shelf === "waiting"
                  ...
                  : {})}
              >
                {shelfThreads.map((row) => cardFor(row, shelf))}
              </Shelf>
            ) : null,
          )}
          <ParkedShelf
        ...
        <ul className="flex flex-col gap-0.5">
          {threads.map((thread) => (
            <SlimRow
              ...
              isActive={thread.id === activeThreadId}
              shelf={shelf}
              statusOverlay={overlayForShelf(shelf)}
